...
export const useGetReferences = (page = 1, limit = 10, filters: ApiFilters | null = null) => {

  const query = useQuery({
    queryKey: ["customers-reference-not-remotely-readable", page, limit, filters],
    queryFn: async () => {
      try {
       ...
       // const response = await axios.post('/api/customers-reference', { accessToken: Cookies.get('access_token') });
        ...
        const response = await axios.post('/api/customers-reference/not-remotely-readable', requestData);
